...
import nodemailer from "nodemailer"; // 🌟 We added nodemailer directly here!
...
export async function GET(req) {
  try {
    ...
    // 🌟 Updated to your actual domain name so email buttons work!
    const appBaseUrl = "https://mof-project-tracker.vercel.app";

    // 🌟 Set up the email sender directly inside the Robot!
    ...
    async function processReminders(col, dateValue, row, sheet, emails, appBaseUrl, isReport) {
      ...
      for (const { address, role } of emails) {
        ...
        if (isReport) {
          ...
          customMessage = `${row.project} - ${col.title} is pending, the last date is ${dateValue}.`;
        } else {
            ...
            html: `
              <div style="font-family: Arial, sans-serif; padding: 20px; border: 1px solid #e2e8f0; border-radius: 8px; max-width: 600px;">
                ...
                <p style="font-size: 12px; color: #94a3b8;">
                  This is an automated notification from the MoF Project Tracker (${sheet.name || "System"}).
                </p>
              </div>
            `,
          };
